...
//			start_time = System.currentTimeMillis();
			...
			for (int j = 0; j < arity; j++) {
				...
				induce(child, subsets[j]);
                    ...
                    //    long stop_time = System.currentTimeMillis();
                	//long elapsed = stop_time - start_time;
            		//if (!recurse) System.out.println("Time: " + elapsed + " arity " +arity);
	...
	public ClusNode induceSingleUnpruned(RowData data, boolean finalRun) throws ClusException, IOException {
		m_Root = null;
		...
		while (true) {
			...
			//if (Settings.m_EnsembleMode) /*VRODIC if is only in IRB version */
                        if (finalRun)
        ...
        public ClusModel induceSingleUnpruned(ClusRun cr, boolean finalRun) throws ClusException, IOException {
		return induceSingleUnpruned((RowData)cr.getTrainingSet(),finalRun);
	}

	public ClusModel induceSingleUnpruned(ClusRun cr) throws ClusException, IOException {
		return induceSingleUnpruned((RowData)cr.getTrainingSet(),false);
	}
}
