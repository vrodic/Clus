...
//	static protected Random m_randTreeDepth = new Random(0);
	...
	public void initialize(ClusSchema schema, Settings sett, Clus clus) throws ClusException, IOException {
            System.out.println("ClusEnsembleInduce::initialize");
		m_BagClus = clus;
...
//		bsimout.appendToFile(m_OForest.getModels(), cr);
	...
	public void induceRForestNoBagging(ClusRun cr) throws ClusException, IOException {
		...
		for (int i = 1; i <= m_NbMaxBags; i++) {
			...
			ClusModel model = ind.induceSingleUnpruned(crSingle, i == m_NbMaxBags);
	...
	public void induceBagging(ClusRun cr) throws ClusException, IOException {
		...
		// bagSelections is either -1, 0, a value in [1,Iterations], or 2 values in [1,Iterations]
                cr.getStatManager().initStatisticAndStatManager();
                cr.getStatManager().initClusteringWeights();

      /*                               FileOutputStream fos = null;
     ObjectOutputStream out = null;
     try
     {
       fos = new FileOutputStream(m_BagClus.getSettings().getFileAbsolute(cr.getStatManager().getSettings().getAppName() + "_cwmain.model"));
       out = new ObjectOutputStream(fos);
       out.writeObject(cr.getStatManager().getClusteringWeights());
       out.close();
     }
...
*/
	...
	public void induceOneBag(ClusRun cr, int i, int origMaxDepth, OOBSelection oob_sel, OOBSelection oob_total, TupleIterator train_iterator, TupleIterator test_iterator, BaggingSelection msel, int threadn) throws ClusException, IOException {   
			...
			// Set random tree max depth
                        // VRODIC this has to be fixed probably if use multithreaded
                        int rand = GDProbl.randDepthWighExponentialDistribution(
//					m_randTreeDepth.nextDouble(),
					ClusRandom.nextDouble(ClusRandom.RANDOM_INT_RANFOR_TREE_DEPTH), origMaxDepth);
                        System.out.println("Tree depth: " + rand);
			getSettings().setTreeMaxDepth(rand);
		}
		...
		if (Settings.shouldEstimateOOB() && (getSettings().getBagSelection().getIntVectorSorted()[0] == -1) && OOBinline){
		...
		if (m_OptMode && OOBinline){                             
			if (i == 1) m_Optimization.initModelPredictionForTuples(model, train_iterator, test_iterator);
                        // VRODIC mark possible lock
			else m_Optimization.addModelPredictionForTuples(model, train_iterator, test_iterator, i);
		}
		if (!m_OptMode) {
			m_OForest.addModelToForest(model);
		...
		//Valid only when test set is supplied
		if (checkToOutEnsemble(i) && (getSettings().getBagSelection().getIntVectorSorted()[0] == -1) && OOBinline){
		...
		if ((getSettings().getBagSelection().getIntVectorSorted()[0] != -1) || (getSettings().isPrintEnsembleModelFiles()) || !OOBinline) {
